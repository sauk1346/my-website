...
<SmartFigure
	...
	src="/inacap/ti3v41/images/fig01.png"
...
- Crear un ambiente virtual "denv1"
	```sh
	python -m venv denv1
	```
 
- Activar ambiente virtual "denv1" (windows)
 	```sh
 	denv1\Scripts\activate
 	```

- Activar ambiente virtual "denv1" (Linux/MacOS)
	```sh
...
- Salir de ambiente virtual (denv1)
	```sh
...
1. Crear proyecto "primerproyecto"
	```sh
...
2. Construir la base de datos, que dará soporte a la aplicación
	```sh
...
2. Crear una vista llamada `index` en `./miapp/views.py`
...
3. Crear `./miapp/urls.py` y configurar `./primerproyecto/urls.py` 
...
4. Agregar `./miapp/` en `./miprimerproyecto/settings.py` para que el proyecto lo reconozca
...
6. El servidor ejecuta la vista `index` en la siguiente ruta:
