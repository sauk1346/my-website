...
1. agregar la vista `muestra_fecha` en `./miapp/views.py`
...
2. Agregar la ruta correspondiente a `./miapp/urls.py`
...
1. Crear la vista `te_saludo` en `./miapp/views.py`
...
2. Agregar la ruta de la vista `te_saludo` en `./miapp/urls.py`
...
3. Ejecutar el servidor y verificar ruta
	```sh
	python manage.py runserver
	```
	...
	```
	- se puede reemplazar **Antonio** por cualquier string
...
1. Crear `proyectotemplate`
	```sh
	django-admin startproject proyectotemplate
	cd proyectotemplate
	```

2. Construir aplicación `miapp`
	```sh
...
4. Agregar `miapp` en `./proyectotemplate/settings.py`
...
5. En `./miapp/`, crear la carpeta `templates` y su subcarpeta `miapp` (la subcarpeta es por buenas prácticas)
	```sh
	mkdir -p ./miapp/templates/miapp/
	```

6. Crear `info.html` en  `./miapp/templates/miapp/info.html`
...
7. Crear la vista `info` para mostrar la página `info.html`
...
8. Crear `./miapp/urls.py` y configurar `./urls.py`
...
9. Ejecutar servidor y verificar ruta
...
1. Modificar la vista `index` ubicada en `./miapp/views.py`
...
1. Modificar la vista `index`, para que acepte `nombre` y `apellido` como parámetros
...
2. Modificar la ruta de la vista `index` en `./miapp/urls.py` a una ruta parametrizada.
...
3. Ejecutar servidor y verificar ruta
