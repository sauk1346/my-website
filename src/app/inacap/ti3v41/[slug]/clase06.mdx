...
7. **Agregar Datos:** Levantar servidor y agregar productos a través del módulo Admin
    ```sh
...
8. **Configurar Templates (1):** Crear directorio `./tiendita/templates/tienda`
    ```sh
    mkdir -p ./tiendita/templates/tiendita
    ```

9. **Configurar Templates (2):** Crear template `index.html`
    ```html
...
11. **Configurar Templates (3):** Agregar los datos creados en la vista `index` (el contexto)
    ```html
    <!-- ./tiendita/templates/tiendita/index.html -->
...
12. **Verificar Datos:** Verificar que se muestran correctamente los datos (contadores) en la ruta de la vista `index`
    ```sh
