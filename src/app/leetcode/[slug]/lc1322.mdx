...
- (ad_id, user_id) es la clave primaria (combination combinación de columnas 
    con valores únicos) para esta tabla.
- Cada fila de esta tabla contiene el ID para un Anuncio, el ID de un usuario, 
    y la acción tomada por este usuario respecto a este Anuncio.
- La columna "action" es un tipo ENUM (categoría) de ('Clicked', 'Viewed', 
    'Ignored')`}
</TerminalOutput>

Una empresa está realizando Anuncios y quiere calcular el rendimiento de cada Anuncio. El rendimiento del Anuncio se mide usando la Tasa de Clics (CTR) donde
...
Nota: no nos importan los Anuncios Ignorados.`}
</TerminalOutput>
