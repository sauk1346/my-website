...
<SmartTable
  tableLayout="auto"
  headers={["Tecnología","Características y Funciones"]}
  rows={[
    ["Python","Es un lenguaje versátil que sirve como \"pegamento\" entre las APIs y MongoDB; Proporciona bibliotecas potentes para trabajar con ambos componentes; Es ampliamente utilizado para el desarrollo backend, ciencia de datos y automatización"],
    ["MongoDB","Python se conecta a MongoDB mediante bibliotecas como `pymongo` o ODMs como `mongoengine`; Perfecta para almacenar datos con estructura flexible como los que suelen provenir de APIs (JSON)"],
    ["APIs","Python permite consumir APIs externas a través de bibliotecas como `requests`; Crear APIs propias con frameworks como Flask, FastAPI o Django REST Framework; Procesar y transformar datos obtenidos de APIs antes de almacenarlos en MongoDB"]
  ]}
/>
...
<SmartTable
  tableLayout="auto"
  headers={["Método HTTP","Equivalente DB","Descripción"]}
  rows={[
