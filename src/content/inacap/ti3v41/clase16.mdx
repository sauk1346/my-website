...
1. **Consumir API:** Crear el método `get_proyectos()` en el módulo `service.py`
    ```python
...
4. **Configurar Templates (1):** Agregar template `proyectos.html`
    ```html
...
5. **Configurar Templates (2):** agregar url de proyectos, en navbar de template `base.html`
    ```html
    <!-- ./proyectoapi/api/templates/base.html -->
    <!DOCTYPE html>
    <html lang="en">
    {% load static %}
    <head>
